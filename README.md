...
<table>
	<thead>
		<tr>
			<th>Вид резервирования</th>
			<th>Описание</th>
			<th>Применение</th>
			<th>Схема резервирования</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>Резервирование ресурсов (CPU, RAM)</td>
			<td>Дублирование ключевых серверов с повышенным потреблением ресурсов</td>
			<td>Обеспечение непрерывной работы при перегрузке основного оборудования</td>
			<td>m = 2 (двойное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование физических компонентов (сервера, диски и т.д.)</td>
			<td>Дублирование критически важных физических компонентов</td>
			<td>Обеспечение доступности данных и систем при отказе оборудования</td>
			<td>m = 3 (тройное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование ДЦ</td>
			<td>Распределение нагрузки между несколькими дата-центрами</td>
			<td>Повышение отказоустойчивости и доступности системы</td>
			<td>m = 4 (четырехкратное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование БД — реплецирование</td>
			<td>Создание копий баз данных на разных серверах</td>
			<td>Обеспечение доступности данных и возможности восстановления после сбоя</td>
			<td>m = 3 (тройное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование API</td>
			<td>Кэширование ответов API на промежуточных серверах</td>
			<td>Уменьшение нагрузки на основные серверы и ускорение ответов</td>
			<td>m = 2 (тройное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование сети</td>
			<td>Дублирование ключевых сетевых соединений</td>
			<td>Обеспечение непрерывности обмена данными между компонентами системы</td>
			<td>m = 3 (двойное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование контейнеров</td>
			<td>Увеличение инстансов контейнеров приложений</td>
			<td>Повышение отказоустойчивости и доступности системы</td>
			<td>m = 3 (тройное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование мониторинга</td>
			<td>Дублирование систем мониторинга и логирования</td>
			<td>Обеспечение непрерывности отслеживания состояния системы</td>
			<td>m = 2 (двойное дублирование)</td>
		</tr>
		<tr>
			<td>Резервирование безопасности</td>
			<td>Дублирование ключевых компонентов безопасности (системы защиты от DDoS-атак, фильтрация malware, вирусные сканы) 
			</td>
			<td>Обеспечение непрерывности защиты данных и систем</td>
			<td>m = 3 (тройное дублирование)</td>
		</tr>
	</tbody>
</table>
...
- Критически важные компоненты (например, авторизация пользователей) выделены в отдельную группу
- Выделение групп серверов по функционалу (например, отдельно группа ленты, отдельно группа мессенджера)
- Отдельные сервисы под сложные функции (например, отдельный сервис для каждого этапа ранжирования)
- Интерфейсы для администрирования и мониторинга изолированы в отдельную группу
- Новый код должен запускаться в отдельной группе
- Тестовые окружения должны быть изолированы от продакшена
- Старые версии компонентов следует постепенно выводить из эксплуатации

### Failover policy

В качестве примера можно разработать следующую политику:
1. Авторизация пользователей (критический):
	- Переключение на запасной источник при первом отказе.
	- Временные рамки: немедленно.
2. База данных (критический):
	- Переключение на резервную копию при втором отказе за минуту.
	- Временные рамки: в течение минуты после первого сбоя.
3. API для публичных сервисов (важный):
	- Переключение на запасной сервер при снижении производительности ниже порога.
	- Временные рамки: в течение 10 минут после обнаружения проблемы.
4. Аналитическая система (необязательный):
	- Переключение на запасной источник при отказе более 1 часа.
	- Временные рамки: в течение часа после подтверждения проблемы.

### Graceful shutdown

Пример graceful shutdown:
1. Предупреждение пользователей:
	- Отправка уведомлений о предстоящем обновлении системы.
	- Информация о времени и причинах закрытия доступа.
2. Сохранение данных:
	- Запуск фонового процесса для сохранения всех изменений в базу данных.
	- Проверка целостности данных перед закрытием.
3. Контроль нагрузки:
	- Медленное ограничение новых запросов для предотвращения перегрузки системы.
	- Перенаправление новых пользователей на альтернативные сервисы.
4. Обработка текущих сессий:
	- Корректное завершение всех активных сессий пользователей.
	- Сохранение состояния сессий перед закрытием.
5. Завершение процессов:
	- Постепенное завершение всех запущенных процессов и задач.
	- Проверка наличия незавершенных операций перед закрытием.
6. Очистка ресурсов:
	- Освобождение памяти и других системных ресурсов.
	- Закрытие открытых файловых каналов и сетевых соединений.
7. Завершение мониторинга:
	- Остановка всех систем мониторинга и логирования.
	- Сохранение метрик о состоянии системы перед закрытием.
8. Проверка целостности:
	- Выполнение дополнительных проверок после завершения работы для подтверждения корректности остановки.

Реализация graceful shutdown:
1. Архитектурный подход:
	- Разработка отдельного сервиса или компонента для управления процессом завершения работы.
	- Использование паттерна "Конечный автомат" для управления состоянием системы во время завершения.
2. Асинхронные операции:
	- Использование асинхронных операций для обеспечения плавности процесса завершения.
	- Применение очередей задач для обработки последних запросов.
3. Таймауты и ограничения:
	- Настройка таймаутов для операций завершения работы.
	- Ограничение времени выполнения каждого этапа процесса завершения.
4. Мониторинг и логирование:
	- Внедрение системы мониторинга для отслеживания состояния во время завершения.
	- Логирование всех ключевых событий процесса завершения.

### Graceful degradation

Пример graceful degradation:
1. Уменьшение объема данных: При высокой нагрузке система постепенно ограничивает объем отображаемых данных на странице. Например, сначала уменьшается количество постов, затем размер изображений, а затем длина комментариев.
2. Ограничение функциональности: При перегрузке серверов система может временно ограничить доступ к некоторым функциям, например, возможность отправлять сообщения или публиковать посты.
3. Упрощение интерфейса: При снижении производительности интерфейс может упроститься, например, уменьшение количества элементов на странице или использование более простых шаблонов.
4. Переадресация запросов: Система может переадресовывать некоторые запросы на альтернативные серверы или базы данных, если основные недоступны.
5. Кэширование данных: При высокой нагрузке система может увеличивать использование кэша для уменьшения запросов к основным источникам данных.
6. Ограничение пользователей: В крайних случаях система может ограничить количество одновременных пользователей или их активности.
7. Сохранение состояния: При необходимости отключения сервера система должна сохранять состояние всех пользовательских действий и продолжать их при восстановлении.

### Асинхронные паттерны

1. CQRS:
	- Разделяет обработки команд и запросов для основных нагруженных компонентов системы (функции постов, комментариев, мессенджера, загрузки и отдача медиа-файлов).
2. Отбивка статистики: 
	- Aсинхронная обработка событий а системе. Используется для отбивки каунтеров (просотры, лайки, подписки, и т.д.)
3. Outbox:
	- Помогает в синхронизации данных между бд, а также увеличивает консистентность за счёт дублирования данных. Используется, например для реализации поиска и ранжирования. Информация о новых постах сохраняется в основную бд, и формируются сообщения, которые отправляются в брокер, а затем записываются в базы для реализации поски и ранжирования.

### Observability

Применение стека ELK

Logstash:
1. Собирание данных:
	- Поддерживает множество источников данных (файлы, базы данных, сетевые протоколы).
	- Может собирать метрики из различных систем мониторинга (Prometheus).
2. Фильтрация и трансформация:
	- Позволяет применять различные фильтры к собранным данным.
	- Поддерживает сложные преобразования и агрегации.
3. Перенаправление данных:
	- Может отправлять данные в Elasticsearch или другие системы.

Elasticsearch:
1. Хранит все собранные логи и метрики.
2. Поиск и анализ:
	- Предоставляет мощные возможности для поиска и анализа логов.
	- Поддерживает сложные запросы и агрегации (Query DSL).
3. Индексация:
	- Автоматически индексирует собранные данные.
	- Обеспечивает быстрый доступ к информации.

Kibana:
1. Визуализация:
	- Создание различных типов графиков, диаграмм и карт.
	- Возможность создания пользовательских панелей мониторинга.
2. Поиск и навигация:
	- Интерактивный поиск по логам.
	- Навигация по временной шкале и структурированным данным.
3. Отчетность:
	- Генерация отчетов и дашбордов.
	- Возможность экспорта данных в различные форматы.
4. Обнаружение аномалий (alert):
	- Автоматическое обнаружение необычной активности.
	- Предупреждение о потенциальных проблемах безопасности.
...
### Расчет веб-серверов NGINX

Расчёт производился, основываясь на информации о выдерживаемом RPS для HTTPS запросов.

- На ~ 300000 RPS понадобится:
	- 300000 / 38900 = 7,7 ~ 8 серверов с CPU 8;
	- 300000 / 77427 = 3,87 ~ 4 сервера с CPU 16;
	- 300000 / 90430 = 3,3 ~ 4 сервера с CPU 32.

Исходя из этих данных разумно взять 4 сервера с CPU 16.
...
- При расчёте количества на 1 ДЦ учтём, что для каждого сервера необходимо резервирование. С учетом 4 ДЦ получаем следующее распределение.
...
- Окупаемость серверов в сравнении с арендой будет достигнута в течение 4 лет.
...
- При расчёте количества сервисов на 1 ДЦ учтём, что для каждого сервиса необходимо резервирование. С учетом 4 ДЦ получаем следующее распределение.
...
**Итого: 21 сервис на 1 кластер и 84 сервиса всего**

### Базы данных

База данных   |	CNT |	CPU  |	RAM/GB  |	ROM/GB
------------- | ------- | --------------- | --------------- | ---------------
Redis         |	16      |	8              |	32           | 20 
Cassandra   |	32      |	24              |	512 	         | 2000
ClickHouse    |	24      |	16              |	64           |	10000
Elasticsearch |	24     |	16              |	64            |	20000
Kafka         |	16      |	16              |	64           |	20000
S3      | 500     | 64              | 512         | 1000000
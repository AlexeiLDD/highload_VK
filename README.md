...
В таблицах _counters и _events собирается информация, которую можно использовать для ранжирования. Организуем ранжирование на основе пулов. Скажем, что в нашем случае это набор уже загруженных в память сгруппированных по определённым критериям объектов, готовых к работе, связанной с выдачей в ленту.

1. Создаем пулы ориентируясь на параметры:
	1. Имея дату создания, запрашиваем только n постов, созданных за последний день, сортируя по дате от новых к старым, дополнительно фильтруя по количеству лайков/комментариев от наименьшего количества к наибольшему. В данном случае получаем группу постов, именуемую «новыми».
...
3. В произвольном запросе мы очень хотим увидеть n постов, отфильтрованных по лайкам и дате добавления, после их ранжирования по значениям полей и по текстовым значениям. 

4. Может возникнуть ситуация, в которой один и тот же пост (в нашем случае информация, которая его описывает на стороне ранжирования) попадёт в больше, чем один пул. Поэтому следует следить за чистотой выдачи. Предположим, что при обработке запросов мы будем отслеживать выдачу из различных пулов через карту, в которой ключом является ID. Если ключ есть, то документ пропускается.

5. Получение информации по первичному ключу из Cassandra.

### Алгоритм поиска
1. Индексация данных:
	- Подготовим данные для индексации (можно воспользоваться предыдущим алгоритмом).
